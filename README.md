<h1 align="center">Hey, I'm Leo 👋</h1>

<p align="center">
  Software developer in <b>Melbourne, Australia</b> - backend APIs, mobile apps,<br>
  and the odd game engine. Most recently on a live Australian mental health platform.
</p>

<p align="center">
  <a href="https://leohngdev.github.io"><img alt="Portfolio" src="https://img.shields.io/badge/Portfolio-leohngdev.github.io-e05561?style=for-the-badge&labelColor=0d1117"></a>
  <a href="https://www.linkedin.com/in/leo-hnguyen"><img alt="LinkedIn" src="https://img.shields.io/badge/LinkedIn-leo--hnguyen-1f6feb?style=for-the-badge&labelColor=0d1117"></a>
  <a href="mailto:hnguyen.leo04@gmail.com"><img alt="Email" src="https://img.shields.io/badge/Email-say%20hello-3fb950?style=for-the-badge&labelColor=0d1117"></a>
</p>

<p align="center">
  <img alt="Open to work" src="https://img.shields.io/badge/open%20to-graduate%20%26%20junior%20software%20roles-e05561?style=flat-square&labelColor=0d1117">
  <img alt="Location" src="https://img.shields.io/badge/based%20in-Melbourne%2C%20AU-30363d?style=flat-square&labelColor=0d1117">
</p>

---

## 🔧 Selected work

| Project | What it is | Status |
| :-- | :-- | :-- |
| **[ANTSA Scoring Engine](https://leohngdev.github.io/work/antsa-scoring-engine/)** | Rebuilt the questionnaire scoring engine behind a live Australian digital mental health platform. Runtime-configurable categories, severity thresholds and weighting. **100% UAT pass across two client-signed iterations.** | `client work` |
| **[PostMint](https://github.com/leohngdev/PostMint)** | Turns a plain-text market take into platform-formatted posts or a narrated 1080×1920 video, through a queue-backed async pipeline. | `in progress` |
| **[Hospitality Till](https://github.com/leohngdev/POS)** | Point-of-sale for a restaurant: PIN auth, dine-in floor map, front-of-house tickets, kitchen chits, staff clock-in. Serves the venue over LAN. | `in a venue` |
| **[leohngdev.github.io](https://github.com/leohngdev/leohngdev.github.io)** | My portfolio site. Astro and Tailwind, no UI framework, performance budgets enforced at build. | `live` |
| **Yard** | A social map app where you plant posts for people nearby to discover. React Native + Expo. | `in development` |

## 🧰 Toolkit

**Backend & data**<br>
![NestJS](https://img.shields.io/badge/NestJS-1b1f23?style=for-the-badge&logo=nestjs&logoColor=e0234e)
![Node.js](https://img.shields.io/badge/Node.js-1b1f23?style=for-the-badge&logo=nodedotjs&logoColor=5fa04e)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-1b1f23?style=for-the-badge&logo=postgresql&logoColor=4169e1)
![MySQL](https://img.shields.io/badge/MySQL-1b1f23?style=for-the-badge&logo=mysql&logoColor=4479a1)
![Docker](https://img.shields.io/badge/Docker-1b1f23?style=for-the-badge&logo=docker&logoColor=2496ed)

**Frontend & mobile**<br>
![React](https://img.shields.io/badge/React-1b1f23?style=for-the-badge&logo=react&logoColor=61dafb)
![React Native](https://img.shields.io/badge/React%20Native-1b1f23?style=for-the-badge&logo=react&logoColor=61dafb)
![Astro](https://img.shields.io/badge/Astro-1b1f23?style=for-the-badge&logo=astro&logoColor=bc52ee)
![Tailwind](https://img.shields.io/badge/Tailwind-1b1f23?style=for-the-badge&logo=tailwindcss&logoColor=06b6d4)
![Expo](https://img.shields.io/badge/Expo-1b1f23?style=for-the-badge&logo=expo&logoColor=ffffff)

**Languages**<br>
![TypeScript](https://img.shields.io/badge/TypeScript-1b1f23?style=for-the-badge&logo=typescript&logoColor=3178c6)
![JavaScript](https://img.shields.io/badge/JavaScript-1b1f23?style=for-the-badge&logo=javascript&logoColor=f7df1e)
![Python](https://img.shields.io/badge/Python-1b1f23?style=for-the-badge&logo=python&logoColor=3776ab)
![Java](https://img.shields.io/badge/Java-1b1f23?style=for-the-badge&logo=openjdk&logoColor=ffffff)
![C++](https://img.shields.io/badge/C%2B%2B-1b1f23?style=for-the-badge&logo=cplusplus&logoColor=00599c)
![C#](https://img.shields.io/badge/C%23-1b1f23?style=for-the-badge&logo=dotnet&logoColor=512bd4)

**Engines & 3D** <sub>coursework</sub><br>
![Unreal](https://img.shields.io/badge/Unreal-1b1f23?style=for-the-badge&logo=unrealengine&logoColor=ffffff)
![Unity](https://img.shields.io/badge/Unity-1b1f23?style=for-the-badge&logo=unity&logoColor=ffffff)
![Maya](https://img.shields.io/badge/Maya-1b1f23?style=for-the-badge&logo=autodeskmaya&logoColor=37a5cc)

<!-- The design's "Lately" section. Both hosts it depends on were failing when this
     was built: github-readme-stats returned 503 on every request, and
     github-readme-activity-graph returned 402 (over quota). Two broken images in the
     middle of the page costs more than the section earns, so it is parked here.

## 📈 Lately

<p align="center">
  <img height="165" alt="Top languages" src="https://github-readme-stats.vercel.app/api/top-langs/?username=leohngdev&layout=compact&hide_border=true&bg_color=0d1117&title_color=e05561&text_color=c9d1d9&langs_count=6">
  <img height="165" alt="Leo's GitHub stats" src="https://github-readme-stats.vercel.app/api?username=leohngdev&show_icons=true&hide_border=true&bg_color=0d1117&title_color=e05561&text_color=c9d1d9&icon_color=3fb950&hide=issues">
</p>
<p align="center">
  <img alt="Contribution graph" src="https://github-readme-activity-graph.vercel.app/graph?username=leohngdev&bg_color=0d1117&color=c9d1d9&line=e05561&point=3fb950&area=true&hide_border=true&custom_title=Commits%20over%20the%20last%20year">
</p>
-->

## 🧠 Behind the code

<details>
<summary><b>How I work</b></summary>

<br>

Software Development major, Games Development minor at Monash. It is an odd pairing and I would choose it again: writing gameplay systems in C++ and C# and optimising topology for realtime rendering made me count frames and bytes, and I still count them when I write a backend.

The two things I am proudest of on ANTSA never shipped as features. I stood the whole multiservice stack up locally before writing a line, then traced a data mismatch that produced no error and no log entry.

</details>

<details>
<summary><b>Before university</b></summary>

<br>

Two years on an FRC robotics team in Ho Chi Minh City, writing navigation and sensor code in Python and C++. Two years of watching a machine do the wrong thing taught me where the work lives: in the gap between what you think a system does and what it does.

</details>

<details>
<summary><b>Away from the keyboard</b></summary>

<br>

Skateboarding, the gym, and games. Different kinds of debugging.

</details>

---

<p align="center">
  <b>Open to graduate and junior software roles in Melbourne.</b><br>
  <a href="mailto:hnguyen.leo04@gmail.com">hnguyen.leo04@gmail.com</a> · <a href="https://leohngdev.github.io">leohngdev.github.io</a>
</p>
